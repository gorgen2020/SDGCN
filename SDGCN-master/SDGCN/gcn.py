...
class gcnCell(RNNCell):
    ...
    def __init__(self, num_units, adj, num_nodes, input_size=None,
        ...
        super(gcnCell, self).__init__(_reuse=reuse)
        self._act = act
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or "gcn"):
    ...
    def _gc(self, inputs, state, output_size, bias=0.0, scope=None):
        ...
        with tf.variable_scope(scope):
            i = 1
            for m in self._adj:
                # x1 = tf.sparse_tensor_dense_matmul(m, x0, adjoint_a=True,adjoint_b=True)
                # print(x1)
